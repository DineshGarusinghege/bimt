...
//                     const CMI = data.data.filter((course) =>
//                         course.program === "CMI(UK) - Management Skill Development Programme"
//                     );

//                     setCourses(CMI);
//                     console.log("Filtered Higher Diploma Courses:", CMI); // Debugging filtered data
//                 }
//             } catch (error) {
...
//                             lineHeight: '53px'
//                         }}
//                     >CMI (UK) Management Skill Development Programme</h2>
//                 </div>
...
export default function CMI() {
        ...
        const fetchCourses = async () => {
            try {
                ...
                if (data.success) {
                    ...
                    setCourses(CMI);
                    console.log("Filtered CMI Courses:", CMI);
                }
            } catch (error) {
                console.error("Failed to fetch courses:", error);
            }
    ...
    //     if (userInput.trim() !== "") {
    //         const filteredSuggestions = courses.filter((course) =>
    //             course.courseName.toLowerCase().includes(userInput.toLowerCase())
    //         );
    //         setSuggestions(filteredSuggestions);
    //     } else {
    //         setSuggestions([]);
    ...
    // const handleSearch = () => {
    //     if (query.trim()) {
    //         const course = courses.find(c => c.courseName.toLowerCase() === query.toLowerCase());
    //         if (course) {
    //             router.push(`/Diploma?course=${encodeURIComponent(query)}`);
    //         } else {
    //             alert("Sorry, this course is not available at the moment. Please contact us for more details.");
    //         }
    //     } else {
    ...
    return (
        <>
            <head>
                ...
                <meta
                    name="description"
                    content="BIMT Campus offers comprehensive Master’s degree programs designed to deepen your expertise and advance your career."
                />
            </head>
            ...
            {/* Hero Section */}
            <div className="relative bg-cover bg-center h-[480px]" style={{ backgroundImage: "url('/images/Rectangle 1.png')" }}>
                <div className="absolute inset-0 bg-black bg-opacity-50"></div>
                <div className="relative max-w-[1430px] mx-auto flex items-center h-full px-6">
                    <nav className="bg-white bg-opacity-70 py-2 px-4 rounded-lg text-sm">
                        <ol className="flex space-x-2 text-gray-700">
                            <li><Link href="/">Home</Link></li>
                            <li>/</li>
                            <li><span className="text-gray-500">CMI (UK) Management Skill Development Programme</span></li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            {/* Courses Section */}
            <div className="bg-[#F6F8FB] py-10 px-6">
                <div className="text-center mb-8">
                    <h3 className="text-lg font-medium text-gray-500">Courses</h3>
                    <h2 className="text-3xl font-bold text-[#1D267D]">CMI (UK) Management Skill Development Programme</h2>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-[1590px] mx-auto">
                    {courses.map((course) => (
                        <div key={course._id} className="bg-white shadow-lg p-6 rounded-lg flex items-center border-l-4 border-[#A02629]">
                            <div className="w-2/3">
                                <h3 className="text-lg font-semibold">{course.courseName}</h3>
                                <p className="text-gray-500">{course.courseTitle}</p>
                                <div className="mt-4 flex space-x-3">
                                    <Link href={`/MSDP`}>
                                        <button className="bg-[#1D267D] text-white px-4 py-2 rounded-md">View more</button>
                                    </Link>
                                    <Link href={'/ContactUs'}>
                                        <button className="bg-transparent border border-[#1D267D] text-[#1D267D] px-4 py-2 rounded-md">Contact Us</button>
                                    </Link>
                                </div>
                            </div>
                            <div className="w-1/3">
                                <Image src={course.courseImg} alt={course.courseName} width={200} height={200} className="rounded-md" />
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="bg-[#006987] flex flex-col items-center py-16 text-white">
                <h3 className="text-xl">Testimonials</h3>
                <h2 className="text-3xl font-bold mb-6">HEAR WHAT OUR ALUMNI SAY ABOUT BIMT</h2>

                <div className="flex items-center space-x-6">
                    <button onClick={handlePrev} className="w-10 h-10 border border-white rounded-full flex items-center justify-center">&lt;</button>
                    
                    <div className="flex flex-col items-center">
                        <Image src={testimonials[currentIndex].image} alt="Alumni" width={200} height={200} className="rounded-md" />
                        <p className="text-lg mt-4 text-center">{testimonials[currentIndex].text}</p>
                        <button className="border border-white px-4 py-2 mt-4">{testimonials[currentIndex].buttonLabel}</button>
                    </div>

                    <button onClick={handleNext} className="w-10 h-10 border border-white rounded-full flex items-center justify-center">&gt;</button>
                </div>
            </div>
        </>
    );
