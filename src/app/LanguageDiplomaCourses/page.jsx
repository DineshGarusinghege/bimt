...
export default function LanguageDiplomaCourses() {
    // Debugging Logs
    useEffect(() => {
        console.log("Debug: studyPrograms:", studyPrograms);
        console.log("Debug: courses:", courses);
    }, []);

    const studyPrograms = [
        { id: 1, title: 'Higher Diploma', img: '/images/Rectangle49-4.png' },
        { id: 2, title: "Bachelor's Degree", img: '/images/Rectangle49-3.png' },
        { id: 3, title: 'Masters Degree', img: '/images/Rectangle49-2.png' },
        { id: 4, title: 'Doctorate Degree', img: '/images/Rectangle49-1.png' },
        { id: 5, title: 'CMI', img: '/images/Rectangle49.png' },
    ] || [];

    const courses = [
        { id: 1, category: 'Diploma', title: 'Advanced Diploma in English', description: 'Part Time', img: '/images/course1.png' },
        { id: 2, category: 'Diploma', title: 'Diploma in English', description: 'Full Time', img: '/images/course2.png' },
    ] || [];

    return (
        <>
            {/* Header */}
            <header className="bg-white shadow relative p-4">
                <div className="container mx-auto flex justify-between items-center">
                    {/* Logo */}
                    <img src="/images/logo1.png" alt="BIMT Campus Logo" width={200} />
                    ...
                    <nav className="hidden md:flex space-x-8 text-sm font-medium text-gray-800">
                        ...
                        <Link href="/life-at-bimt" className="hover:text-gray-600">Life at BIMT</Link>
                        <Link href="/contact" className="hover:text-gray-600">Connect with Us</Link>
                    </nav>

                    {/* Contact Info */}
                    <div className="flex items-center space-x-4">
                        <span>📞 +94 115 33 22 22</span>
                        <span>📞 +94 777 92 59 92</span>
                    </div>
                </div>
            </header>

            {/* Announcement Bar */}
            <div className="bg-[#A02629] text-white text-center py-2">
                A workshop on Capital Market was organized for our BIMT Campus Students at Kingsbury Hotel Colombo.
            </div>

            {/* Hero Section */}
            <div className="relative bg-cover bg-center h-[500px] flex items-center justify-center text-white" style={{ backgroundImage: "url('/images/Rectangle1.png')" }}>
                <h1 className="text-4xl font-bold">Language Diploma Courses</h1>
            </div>

            {/* Courses Section */}
            <div className="bg-[#F6F8FB] py-10 px-4">
                <div className="text-center mb-8">
                    <h3 className="text-lg font-medium text-gray-500">Courses</h3>
                    <h2 className="text-3xl font-bold text-[#272A5D]">Language Diploma Courses</h2>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-[1200px] mx-auto">
                    {courses.length > 0 ? courses.map((course) => (
                        <div key={course.id} className="flex bg-white shadow-lg overflow-hidden border-l-4 border-[#A02629]">
                            <div className="p-4 w-2/3">
                                <p className="text-sm text-gray-500">{course.category}</p>
                                <h3 className="text-xl font-semibold text-[#1D267D]">{course.title}</h3>
                                <h4 className="text-lg text-gray-600">{course.description}</h4>
                                <div className="mt-4 space-x-4">
                                    <button className="border border-[#1D267D] px-4 py-2 text-[#1D267D] hover:bg-[#1D267D] hover:text-white">View more</button>
                                    <button className="border border-[#1D267D] px-4 py-2 bg-[#1D267D] text-white hover:bg-[#151B54]">Contact Us</button>
                                </div>
                            </div>
                            <div className="w-1/3">
                                <Image src={course.img || "/images/default.png"} alt={course.title} width={200} height={200} />
                            </div>
                        </div>
            ...
            <div className="bg-[#F6F8FB] py-10 px-4">
                <div className="text-center mb-8">
                    <h3 className="text-lg font-medium text-gray-500">Study</h3>
                    <h2 className="text-3xl font-bold text-[#272A5D]">We Offer to You</h2>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 max-w-[1200px] mx-auto">
                    {studyPrograms.length > 0 ? studyPrograms.map((study) => (
                        <div key={study.id} className="bg-white border shadow-md rounded-lg overflow-hidden hover:shadow-lg">
                            <div className="w-full h-[200px]">
                                <Image src={study.img || "/images/default.png"} alt={study.title} width={200} height={200} />
                            </div>
                            <div className="p-4 text-center">
                                <h3 className="text-lg font-semibold text-[#1D267D]">{study.title}</h3>
                                <button className="mt-4 border border-[#1D267D] px-4 py-2 text-[#1D267D] hover:bg-[#1D267D] hover:text-white">View more</button>
                            </div>
                        </div>
                    )) : <p className="text-center">No study programs available</p>}
                </div>
            </div>
            ...
            <div className="bg-[#006987] py-16 px-6 text-white text-center">
                ...
                <p className="text-xl mt-4">"The continuous support, help, and guidance given by the campus management helped me achieve my goals!"</p>
